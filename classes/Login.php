...
class Login
{
    ...
    private function databaseConnection()
    {
        ...
        if ($this->db_connection != null) {
            ...
            try {
                ...
                $this->errors[] = '<font color="red">' . $lang['MESSAGE_DATABASE_ERROR'] . '</font><br>' . $e->getMessage();
            }
        }
        // default return
        return false;
    ...
    private function getUserData($user_name)
    {
        ...
        if ($this->databaseConnection()) {
            ...
            $query_user = $this->db_connection->prepare('SELECT * FROM tbl_users WHERE user_name = :user_name');
        ...
        if (isset($_COOKIE['rememberme'])) {
            ...
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)) {
                ...
                if ($this->databaseConnection()) {
                    ...
                    $sth = $this->db_connection->prepare("SELECT user_id, user_name, user_email FROM tbl_users WHERE user_id = :user_id
            ...
            $this->errors[] = '<font color="red">' . $lang['MESSAGE_COOKIE_INVALID'] . '</font><br>';
        }
        return false;
    }
    ...
    private function loginWithPostData($user_name, $user_password, $user_rememberme)
    {
        global $lang;
        if (empty($user_name)) {
            $this->errors[] = '<font color="red">' . $lang['MESSAGE_USERNAME_EMPTY'] . '</font><br>';
        } else if (empty($user_password)) {
            $this->errors[] = '<font color="red">' . $lang['MESSAGE_PASSWORD_EMPTY'] . '</font><br>';
        ...
        // if POST data (from login form) contains non-empty user_name and non-empty user_password
            ...
            // if user has typed a valid email address, we try to identify him with his user_email
                ...
                // database query, getting all the info of the selected user
                $query_user = $this->db_connection->prepare('SELECT * FROM tbl_users WHERE user_email = :user_email');
            ...
            if (! isset($result_row->user_id)) {
                ...
                $this->errors[] = '<font color="red">' . $lang['MESSAGE_LOGIN_FAILED'] . '</font><br>';
            } else if (($result_row->user_failed_logins >= 3) && ($result_row->user_last_failed_login > (time() - 30))) {
                $this->errors[] = '<font color="red">' . $lang['MESSAGE_PASSWORD_WRONG_3_TIMES'] . '</font><br>';
            // using PHP 5.5's password_verify() function to check if the provided passwords fits to the hash of that user's password
            } else if (! password_verify($user_password, $result_row->user_password_hash)) {
                // increment the failed login counter for that user
                $sth = $this->db_connection->prepare('UPDATE tbl_users '
                        . 'SET user_failed_logins = user_failed_logins+1, user_last_failed_login = :user_last_failed_login '
                        . 'WHERE user_name = :user_name OR user_email = :user_name');
                $sth->execute(array(':user_name' => $user_name, ':user_last_failed_login' => time()));

                $this->errors[] = '<font color="red">' . $lang['MESSAGE_PASSWORD_WRONG'] . '</font><br>';
            // has the user activated their account with the verification email
            } else if ($result_row->user_active != 1) {
                $this->errors[] = '<font color="red">' . $lang['MESSAGE_ACCOUNT_NOT_ACTIVATED'] . '</font><br>';
            } else {
                // write user data into PHP SESSION [a file on your server]
                $_SESSION['user_id'] = $result_row->user_id;
                $_SESSION['user_name'] = $result_row->user_name;
                ...
                if (defined('HASH_COST_FACTOR')) {
                    ...
                    if (password_needs_rehash($result_row->user_password_hash, PASSWORD_DEFAULT, array('cost' => HASH_COST_FACTOR))) {
                        ...
                        $query_update = $this->db_connection->prepare('UPDATE tbl_users SET user_password_hash = :user_password_hash WHERE user_id = :user_id');
    ...
    private function newRememberMeCookie()
    {
        ...
        if ($this->databaseConnection()) {
            ...
            $sth = $this->db_connection->prepare("UPDATE tbl_users SET user_rememberme_token = :user_rememberme_token WHERE user_id = :user_id");
    ...
    private function deleteRememberMeCookie()
    {
        ...
        if ($this->databaseConnection()) {
            ...
            $sth = $this->db_connection->prepare("UPDATE tbl_users SET user_rememberme_token = NULL WHERE user_id = :user_id");
    ...
    public function doLogout()
    {
        ...
        //print("In public function doLogout in Class Login.php<br />");
        $this->messages[] = '<font color="red">' . $lang['MESSAGE_LOGGED_OUT'] . '</font><br>';

        //$this->lang[] = MESSAGE_AIM;
        //echo $lang['MESSAGE_LOGGED_OUT'];
        //$this->messages[] = ($lang['MESSAGE_LOGGED_OUT']);
    ...
    public function editUserName($user_name)
    {
        ...
        if (!empty($user_name) && $user_name == $_SESSION['user_name']) {
            $this->errors[] = '<font color="red">' . $lang['MESSAGE_USERNAME_SAME_LIKE_OLD_ONE'] . '</font><br>';
        ...
        // TODO: maybe this pattern should also be implemented in Registration.php (or other way round)
            ...
            $this->errors[] = '<font color="red">' . $lang['MESSAGE_USERNAME_INVALID'] . '</font><br>';
            ...
            if (isset($result_row->user_id)) {
                ...
                $query_edit_user_name = $this->db_connection->prepare('UPDATE tbl_users SET user_name = :user_name WHERE user_id = :user_id');
                ...
                if ($query_edit_user_name->rowCount()) {
                    ...
                    $this->messages[] = '<font color="red">' . $lang['MESSAGE_USERNAME_CHANGED_SUCCESSFULLY'] . '</font>' . '<a>' . $user_name .'</a><hr/>';
                } else {
                    $this->errors[] = '<font color="red">' . $lang['MESSAGE_USERNAME_CHANGE_FAILED'] . '</font><br>';
                }
            }
        }
    }
    ...
    public function editUserEmail($user_email)
    {
        ...
        if (!empty($user_email) && $user_email == $_SESSION["user_email"]) {
            $this->errors[] = '<font color="red">' . $lang['MESSAGE_EMAIL_SAME_LIKE_OLD_ONE'] . '</font><br>';
        // user mail cannot be empty and must be in email format
        } elseif (empty($user_email) || !filter_var($user_email, FILTER_VALIDATE_EMAIL)) {
            $this->errors[] = '<font color="red">' . $lang['MESSAGE_EMAIL_INVALID'] . '</font><br>';
            ...
            if (isset($result_row->user_id)) {
                $this->errors[] = '<font color="red">' . $lang['MESSAGE_EMAIL_ALREADY_EXISTS'] . '</font><br>';
            } else {
                // write users new data into database
                $query_edit_user_email = $this->db_connection->prepare('UPDATE tbl_users SET user_email = :user_email WHERE user_id = :user_id');
                ...
                if ($query_edit_user_email->rowCount()) {
                    ...
                    $this->messages[] = '<font color="red">' . $lang['MESSAGE_EMAIL_CHANGED_SUCCESSFULLY'] . '</font><br>' . $user_email;
                } else {
                    $this->errors[] = '<font color="red">' . $lang['MESSAGE_EMAIL_CHANGE_FAILED'] . '</font><br>';
                }
            }
        }
    }
    ...
    public function editUserPassword($user_password_old, $user_password_new, $user_password_repeat)
    {
        ...
        // is the repeat password identical to password
            ...
            $this->errors[] = '<font color="red">' . $lang['MESSAGE_PASSWORD_BAD_CONFIRM'] . '</font><br>';
        // password need to have a minimum length of 6 characters
        } elseif (strlen($user_password_new) < 6) {
            $this->errors[] = '<font color="red">' . $lang['MESSAGE_PASSWORD_TOO_SHORT'] . '</font><br>';
            ...
            if (isset($result_row->user_password_hash)) {
                ...
                if (password_verify($user_password_old, $result_row->user_password_hash)) {
                    ...
                    if ($query_update->rowCount()) {
                        $this->messages[] = '<font color="red">' . $lang['MESSAGE_PASSWORD_CHANGED_SUCCESSFULLY'] . '</font><br>';
                    } else {
                        $this->errors[] = '<font color="red">' . $lang['MESSAGE_PASSWORD_CHANGE_FAILED'] . '</font><br>';
                    }
                } else {
                    $this->errors[] = '<font color="red">' . $lang['MESSAGE_OLD_PASSWORD_WRONG'] . '</font><br>';
                }
            } else {
                $this->errors[] = '<font color="red">' . $lang['MESSAGE_USER_DOES_NOT_EXIST'] . '</font><br>';
            }
        }
    }
    ...
    public function setPasswordResetDatabaseTokenAndSendMail($user_name)
    {
        ...
        if (empty($user_name)) {
            ...
            if (isset($result_row->user_id)) {
                ...
                // database query:
                $query_update = $this->db_connection->prepare('UPDATE tbl_users SET user_password_reset_hash = :user_password_reset_hash,
                ...
                if ($query_update->rowCount() == 1) {
                    ...
                    $this->errors[] = '<font color="red">' . $lang['MESSAGE_DATABASE_ERROR'] . '</font><br>';
                }
            } else {
                $this->errors[] = '<font color="red">' . $lang['MESSAGE_USER_DOES_NOT_EXIST'] . '</font><br>';
            }
    ...
    public function sendPasswordResetMail($user_name, $user_email, $user_password_reset_hash)
    {
        ...
        if(!$mail->Send()) {
            $this->errors[] = '<font color="red">' . $lang['MESSAGE_PASSWORD_RESET_MAIL_FAILED'] . '</font><br>' . $mail->ErrorInfo;
            return false;
        } else {
            $this->messages[] = '<font color="red">' . $lang['MESSAGE_PASSWORD_RESET_MAIL_SUCCESSFULLY_SENT'] . '</font><br>';
            return true;
        }
    }
    ...
    public function checkIfEmailVerificationCodeIsValid($user_name, $verification_code)
    {
        ...
        if (empty($user_name) || empty($verification_code)) {
            $this->errors[] = '<font color="red">' . $lang['MESSAGE_LINK_PARAMETER_EMPTY'] . '</font><br>';
        } else {
            // database query, getting all the info of the selected user
            $result_row = $this->getUserData($user_name);
            ...
            if (isset($result_row->user_id) && $result_row->user_password_reset_hash == $verification_code) {
                ...
                if ($result_row->user_password_reset_timestamp > $timestamp_one_hour_ago) {
                    ...
                    $this->errors[] = '<font color="red">' . $lang['MESSAGE_RESET_LINK_HAS_EXPIRED'] . '</font><br>';
                }
            } else {
                $this->errors[] = '<font color="red">' . $lang['MESSAGE_USER_DOES_NOT_EXIST'] . '</font><br>';
            }
    ...
    public function editNewPassword($user_name, $user_password_reset_hash, $user_password_new, $user_password_repeat)
    {
        ...
        // if database connection opened
            ...
            // write users new hash into database
            $query_update = $this->db_connection->prepare('UPDATE tbl_users SET user_password_hash = :user_password_hash,
